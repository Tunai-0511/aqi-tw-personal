...
# 通用配色盤 — 24 色,專為深色背景挑選的高對比、彼此明顯區隔的顏色。
# 設計目的:24h 趨勢圖最多會同時顯示 22 個城市的曲線,過去 10 色會循環撞色,
# 使用者選了「全選」就分不清哪條線是哪個城市。改成 24 色後不會在 22 城市內 wrap。
# 排序原則:先放主題色(青/橘/綠/黃/紫/紅),再交錯插入其他色相,讓相鄰兩條線
# 的色調差異盡量大,降低視覺混淆。
PALETTE = [
    CYAN,        # #00d9ff 青
    ORANGE,      # #ff8c42 橘
