# SKILL.md — AQI Live（讀 LobsterAQI 匯出）

## Purpose

讓 Hermes(Discord bot)回答台灣空品問題,資料來自 LobsterAQI Pipeline 匯出的
`hermes_export/latest_aqi.json`。這是**拉取(pull)模型** —— 不是 LobsterAQI 推給
Hermes,而是 Hermes「來這裡讀」最新一次 Pipeline 的結果。

## 資料來源

`<LobsterAQI 專案根>/hermes_export/latest_aqi.json`,每次使用者在 LobsterAQI 跑一次
Pipeline 就更新(UTF-8 JSON)。路徑可用環境變數 `LOBSTER_EXPORT` 覆寫。
...
| `cities[]` | 20 城市 `{city, region, aqi, level, pm25, pm10, o3, no2, so2, co, risk}` |
| `analyst_summary` | 分析師風險摘要(純文字) |
| `advisories` | `{城市名: 預警員建議文字}` |
| `user_profile` | `{age, sex, bmi, diagnoses[], conditions[], med_history, city, threshold}` 或 `null` |
