"""
aqi-live skill reader — 讀 LobsterAQI 匯出的 latest_aqi.json,印出可貼進 Discord 的
繁中空品摘要(全國概況 / 指定城市 + 依 user_profile 個人化)。

這是「拉取模型」的 Hermes 端:LobsterAQI 跑完 Pipeline 把結果寫進
...
路徑:預設讀 <專案根>/hermes_export/latest_aqi.json,可用環境變數 LOBSTER_EXPORT 覆寫。
"""
...
def export_path() -> Path:
    """latest_aqi.json 的路徑:優先用 LOBSTER_EXPORT 環境變數,否則專案根的 hermes_export/。"""
    env = os.environ.get("LOBSTER_EXPORT")
    if env:
        return Path(env)
...
def main() -> None:
    ...
    if not path.exists():
        print(
            f"找不到匯出檔:{path}\n"
            "請先在 LobsterAQI 跑一次 Pipeline 產生 latest_aqi.json(或設 LOBSTER_EXPORT)。"
        )
        sys.exit(1)
    try:
