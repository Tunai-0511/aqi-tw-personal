...
.office {
  ...
  display: grid;
  /* Force 3 in a row so the three bots line up neatly across the
...
.office::before {
  content: 'PIXEL OFFICE · AGENT BOTS';
...
.desk {
  ...
  flex-direction: column;
  align-items: center;
  justify-content: flex-end;          /* anchor bot + base to the bottom */
  padding-top: 1rem;
...
.bubble {
  ...
  font-size: 0.7rem;
  color: #e8eef7;
  /* Tight gap below so the speech-tail visually meets the bot's head
...
.bubble::after {
  ...
  /* Tail extends slightly past the bubble so it sits between bubble and
     the bot's head — not the desk monitor box below. */
  bottom: -7px; left: 50%;
...
/* Empty state — fully hidden, takes no visual space (no faint outline,
   no margin). The desk's flex layout keeps the bot anchored at the
   bottom regardless. */
.bubble.empty {
  opacity: 0;
...
.bot {
  ...
  /* Lift the emoji a hair so the speech-bubble tail visually lands on the
     bot's head, not on empty space above it. */
  margin-top: -0.25rem;
}
.bot.active {
  filter: none;
...
.agent-desc {
  ...
  /* Reserve 2 lines of description height so desks whose desc happens to
     ...
     flex-end` on .desk pushes the shorter desk's bot downward. */
