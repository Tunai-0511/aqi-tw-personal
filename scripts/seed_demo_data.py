...
灌入內容(全部明確標記為 demo,放在隔離的 source,可一鍵清除,不污染真實使用):
  ...
  3) 常駐城市過去 14 天的健康日誌
       → 寫進 SQLite `health_diary`,note 前綴 "[demo]"
       → 症狀分數由「同一條軌跡」推導 → SECTION 09 散點 Pearson r 明顯為正
...
⚠ 為何要標記 + 隔離:本專案一向強調「真實外部 API、無合成資料」,且使用者可能在
  ...
  - 日誌:note 內含 "[demo]" 標記;散點以 source='demo' 對齊 AQI。
  兩者都可用 `python seed_demo_data.py --clear` 一鍵移除(刪 data_mode='demo')。

個人健康檔案(年齡 / BMI / ICD-10 / 病歷)是 Streamlit session_state,無法由本
腳本預先灌入 —— 請在 App 封面「步驟① 個人健康檔案」按「🎬 載入我的檔案」一鍵填入。
...
DIARY_DAYS = 14               # 健康日誌天數 = 常駐城市「污染事件」軌跡長度
...
_SYMPTOM_BASE = 40            # symptom = (當日設計均值 - base) / span (+ jitter)
_SYMPTOM_SPAN = 18
...
def seed_health_diary(city_id: str, today) -> int:
    """灌入常駐城市過去 DIARY_DAYS 天健康日誌(症狀由同一條 DAILY_TARGETS 推導 → 正相關)。"""
    written = 0
    for idx in range(DIARY_DAYS, 0, -1):           # 最舊 → 最新(不含今天,留給現場打卡 demo)
        day = today - timedelta(days=idx)
        target = DAILY_TARGETS[DIARY_DAYS - idx]
        # 症狀:由當日設計均值線性推導 + 可重現 jitter(讓 r 明顯為正但 < 1,看起來寫實)
        jitter = ((idx * 37) % 5 - 2) * 0.35       # 約 [-0.7, +0.7],由 idx 決定 → 可重現
        symptom = max(0, min(5, round((target - _SYMPTOM_BASE) / _SYMPTOM_SPAN + jitter)))
        # 戶外時數:症狀越重越少出門(寫實反向關係)+ 一點天與天變化
        outdoor = max(30, min(220, int(200 - symptom * 30 + (idx % 3) * 12)))
        note = f"{DEMO_DIARY_TAG} 設計每日均值 AQI≈{target:.0f}"
        tsdb.upsert_diary_entry(
            date=day.isoformat(), city_id=city_id,
            symptom_score=symptom, outdoor_min=outdoor, note=note,
        )
        written += 1
    return written
...
def main() -> None:
    parser = argparse.ArgumentParser(description="LobsterAQI 期末 demo 假資料灌庫工具")
    parser.add_argument("--clear", action="store_true", help="清除所有 demo 資料後結束")
    parser.add_argument("--city", default="taipei", help="健康日誌的常駐城市 id(預設 taipei)")
    args = parser.parse_args()

    print("=" * 60)
    print("  LobsterAQI · 期末 demo 假資料灌庫工具")
    ...
    print(f"  ③ 灌入「{city_name}」過去 {DIARY_DAYS} 天健康日誌 …")
    n_diary = seed_health_diary(city_id, today)
    print(f"     ✓ 寫入 {n_diary} 筆(note 標記 {DEMO_DIARY_TAG})")

    print("-" * 60)
    print("  ✓ demo 資料就緒!現在可以:")
    print("    - SECTION 08:看「7 天趨勢圖」起伏 +「比上週」明顯 ↑ 徽章")
    print("    - 封面步驟①:按「🎬 載入我的檔案」填入本人 persona")
    print("    - SECTION 09:看「症狀 vs AQI」相關性散點 + Pearson r(明顯為正)")
